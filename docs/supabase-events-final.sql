-- Script FINAL CORRIGIDO para ajustar tabelas de eventos no Supabase
-- Solução para todos os erros identificados
...
-- 2. Adicionar colunas necessárias na tabela events
...
-- 3. Atualizar constraint do event_type para incluir 'inaugural'
DO $$
BEGIN
...
-- 11. RLS Policies CORRIGIDAS (sem usar p.email que não existe)
...
-- Policy CORRIGIDA para responsáveis verem apenas eventos de seus alunos
-- Usar auth.users.email em vez de p.email que não existe
...
-- Policies CORRIGIDAS para event_participants
...
-- Policy CORRIGIDA para participações
